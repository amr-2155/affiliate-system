...
export default function HeaderNotifications() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <Bell size={19} className={ringing ? "animate-bell-ring" : ""} />
        {unreadCount > 0 && (
          <span key={unreadCount} className="absolute top-1.5 left-1.5 min-w-[18px] h-[18px] bg-gradient-to-r from-red-500 to-rose-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1 shadow-sm animate-badge-pop">
                ...
                return (
                  <div
                    ...
                    className={`group flex items-start gap-3 px-4 py-3 border-b border-slate-50 transition-colors duration-150 cursor-pointer relative ${
                      n.read ? "opacity-75 hover:bg-slate-50" : "hover:bg-blue-50/50"
                    }`}
                    title={n.title}
                  >
